...
"""Query-mode (#?) operations as a SQLTranslator base class.

The ``_query`` dispatcher and its helpers (``_paginated_jmd``,
``_aggregate``, ``_aggregate_join``, ``_query_with_joins``,
``_build_col_namespace``) live here as :class:`_QueryOpsBase`.
:class:`SQLTranslator` inherits from it. Same pattern as
:mod:`_schema_ops`: methods stay methods, the base class declares
the attributes and helper methods the concrete translator
supplies, mypy strict resolves everything through the MRO.
"""
...
# Mirrors the constant of the same name in translator.py; kept
# duplicated rather than imported to break the otherwise-circular
# translator → query_ops → translator dependency.
...
class _QueryOpsBase:
    """Base class providing query (#?) operations for SQLTranslator.

    Same split pattern as :class:`_SchemaOpsBase`: the runtime
    attributes (``_conn``, ``_schema``) and the helper methods this
    base calls into (``_resolve_or_error``, ``_build_where_from_fields``,
    ``_fetchall``, ``_explain``) are concretely supplied by
    :class:`SQLTranslator`. The declarations here give mypy strict
    the surface it needs to type-check the bodies.
    """

    _conn: sqlite3.Connection
    _schema: SchemaInspector

    # Concrete on SQLTranslator; declared here so mypy resolves the
    # calls inside the query bodies. Bodies raise to fail fast on
    # accidental direct use of _QueryOpsBase.
    ...
    ) -> tuple[str, list[Any]]:  # pragma: no cover
        """Translate QBE fields into a SQL WHERE fragment."""
        raise NotImplementedError
    ...
    def _explain(
        ...
        """Return SQLite's query plan for *sql*."""
        raise NotImplementedError
